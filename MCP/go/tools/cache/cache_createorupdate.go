...
func Cache_createorupdateHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		cacheIdVal, ok := args["cacheId"]
		if !ok {
			return mcp.NewToolResultError("Missing required path parameter: cacheId"), nil
		}
		cacheId, ok := cacheIdVal.(string)
		if !ok {
			return mcp.NewToolResultError("Invalid path parameter: cacheId"), nil
		}
		...
		url := fmt.Sprintf("%s/subscriptions/%s/resourceGroups/%s/providers/Microsoft.ApiManagement/service/%s/caches/%s%s", cfg.BaseURL, resourceGroupName, serviceName, cacheId, subscriptionId, queryString)
...
func CreateCache_createorupdateTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_subscriptions_subscriptionId_resourceGroups_resourceGroupName_providers_Microsoft_ApiManagement_service_serviceName_caches_cacheId",
		mcp.WithDescription("Creates or updates an External Cache to be used in Api Management instance."),
		...
		mcp.WithString("cacheId", mcp.Required(), mcp.Description("Identifier of the Cache entity. Cache identifier (should be either 'default' or valid Azure region identifier).")),
		mcp.WithString("If-Match", mcp.Description("ETag of the Entity. Not required when creating an entity, but required when updating an entity.")),
		...
		mcp.WithString("type", mcp.Description("Input parameter: Resource type for API Management resource.")),
		mcp.WithString("properties", mcp.Description("Input parameter: Properties of the Cache contract.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Cache_createorupdateHandler(cfg),
	}
}
