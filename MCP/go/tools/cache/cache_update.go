...
func Cache_updateHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateCache_updateTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("patch_subscriptions_subscriptionId_resourceGroups_resourceGroupName_providers_Microsoft_ApiManagement_service_serviceName_caches_cacheId",
		mcp.WithDescription("Updates the details of the cache specified by its identifier."),
		...
		mcp.WithString("properties", mcp.Description("Input parameter: Parameters supplied to the Update Cache operation.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Cache_updateHandler(cfg),
	}
}
