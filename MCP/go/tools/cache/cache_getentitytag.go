...
func Cache_getentitytagHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateCache_getentitytagTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("head_subscriptions_subscriptionId_resourceGroups_resourceGroupName_providers_Microsoft_ApiManagement_service_serviceName_caches_cacheId",
		mcp.WithDescription("Gets the entity state (Etag) version of the Cache specified by its identifier."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Cache_getentitytagHandler(cfg),
	}
}
